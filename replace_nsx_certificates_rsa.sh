...
if [ -e $file ]; then
  ...
  openssl req -x509 -new -nodes -key ca.key -sha256 -days 3650 -out ca.crt -subj "/CN=vAPP-CA" -extensions v3_ca
fi
...
if [ -z $TN_ID ]; then
  ...
  if [ -e $file ]; then
    ...
    openssl x509 -req -in $TN_ID'.csr' -CA ca.crt -CAkey ca.key -CAcreateserial -out $TN_ID'.crt' -days 3650 -sha256 -extfile <(printf "extendedKeyUsage=serverAuth,clientAuth")
  fi
...
if [ -e $file ]; then
  ...
  echo "Generate a private key for the service"
  openssl genrsa -out $SERVICE_TYPE'.key' 2048 -noout

  echo "Create a CSR for the service"
  openssl req -new -key $SERVICE_TYPE'.key' -out  $SERVICE_TYPE'.csr' -subj "/CN="$SERVICE_TYPE -extensions client_server_ssl -config <(cat /etc/ssl/openssl.cnf <(printf '[client_server_ssl]\nextendedKeyUsage = serverAuth,clientAuth\n'))

  echo "Sign the CSR with the CA"
  openssl x509 -req -in $SERVICE_TYPE'.csr' -CA ca.crt -CAkey ca.key -CAcreateserial -out $SERVICE_TYPE'.crt' -days 3650 -sha256 -extfile <(printf "extendedKeyUsage=serverAuth,clientAuth")
fi
